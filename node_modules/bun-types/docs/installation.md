...
$ curl -fsSL https://bun.sh/install | bash -s "bun-v1.2.11"
```
...
To install a specific version of Bun, you can pass the git tag of the version you want to install to the install script, such as `bun-v1.2.0` or `bun-v1.2.11`.
...
```sh
# PowerShell:
$ iex "& {$(irm https://bun.sh/install.ps1)} -Version 1.2.11"
```
